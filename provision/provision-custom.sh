...
# Custom provisioner
#
# The minimal amount of changes are done so that diff'ing against the canonical file is easier
#
# TODO: Merge changes to provision.sh whenever they're made, otherwise they won't have any effect
#
...
if [[ $ping_result == "Connected" ]]; then
	...
	if [[ ! -d /srv/tools/wp-cli ]]; then
		...
		git clone https://github.com/wp-cli/wp-cli.git /srv/tools/wp-cli
		cd /srv/tools/wp-cli
		composer install
	else
		echo -e "\nUpdating wp-cli..."
	...
	if [[ ! -d /srv/tools/phpcs ]]; then
		...
		git clone -b master https://github.com/squizlabs/PHP_CodeSniffer.git /srv/tools/phpcs
	else
		cd /srv/tools/phpcs
	...
	if [[ ! -d /srv/tools/phpcs/CodeSniffer/Standards/WordPress ]]; then
		...
		git clone -b master https://github.com/WordPress-Coding-Standards/WordPress-Coding-Standards.git /srv/tools/phpcs/CodeSniffer/Standards/WordPress
	else
	...
	/srv/tools/phpcs/scripts/phpcs --config-set installed_paths ./CodeSniffer/Standards/WordPress/
	/srv/tools/phpcs/scripts/phpcs --config-set default_standard WordPress-Core
	/srv/tools/phpcs/scripts/phpcs -i
