...
#PHP
	...
	if [[ ! -d /srv/www/wp-develop.dev ]]; then
...
if ( 'build' == basename( dirname( __FILE__) ) ) {
	...
	#define( 'WP_SITEURL', 'http://build.wp-develop.dev.dev' );
}

define( 'WP_DEBUG', true );
PHP
		wp core install --url=wp-develop.dev --quiet --title="WordPress Develop" --admin_name=iandunn --admin_email="ian@iandunn.name" --admin_password="password" --allow-root
		cp /srv/config/wordpress-config/wp-tests-config.php /srv/www/wp-develop.dev/
		cd /srv/www/wp-develop.dev/
	...
	if [[ ! -d /srv/www/wp-develop.dev/build ]]; then
